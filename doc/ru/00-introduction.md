...
**Требуют root** и пакет `bcc-tools`. Эти инструменты дают распределения задержек (гистограммы), которые procfs не может предоставить.
...
**Требует root** и современное ядро (≥ 5.8 с поддержкой BTF). Это будущее Linux-наблюдаемости.

## Профили сбора данных

sysdiag поддерживает три профиля, контролирующих время сбора и набор инструментов:

| Профиль | Длительность | Когда использовать |
|---------|-------------|-------------------|
| `quick` | 10 секунд | Быстрая проверка, CI/CD |
| `standard` | 30 секунд | Регулярная диагностика |
| `deep` | 60 секунд | Поиск коренной причины, включая стеки |
